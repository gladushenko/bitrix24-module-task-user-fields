	...
	implements \Bitrix\Main\Errorable, \Bitrix\Main\Engine\Contract\Controllerable
{
	...
	public function getTotalCountAction($userId, $groupId, $parameters)
	{
		...
		$taskFilter = Filter::getInstance($userId, $groupId, $gridId);
		$filterRows = static::patchTaskUserFieldsFilterRows($taskFilter->getFilters(), $groupId);
		$filter = static::patchTaskUserFieldsProcessedFilter(
			$taskFilter->process(),
			$taskFilter->getOptions()->getFilter($filterRows),
			$groupId
		);
		if ($relationToId)
		{
	...
	public static function getNearTasks(array $taskIds, array $navigation, array $arParams = []): array
	{
		...
		$filterRows = static::patchTaskUserFieldsFilterRows($filter->getFilters(), $groupId);
		...
		$getListParameters = [
			'select' => ['ID'],
			'legacyFilter' => static::patchTaskUserFieldsProcessedFilter(
				$filter->process(),
				$filter->getOptions()->getFilter($filterRows),
				$groupId
			),
	...
	protected function doPreAction()
	{
		...
		$this->arResult["FILTER"] = static::patchTaskUserFieldsFilterRows(
		...
		$this->listParameters['filter'] = ($this->arParams['IS_MOBILE'] ?? null)
			...
			: $this->filter->process()
		; //TODO!
		$this->listParameters['filter'] = static::patchTaskUserFieldsProcessedFilter(
			$this->listParameters['filter'],
			$this->filter->getOptions()->getFilter($this->arResult["FILTER"]),
			(int)($this->arParams['GROUP_ID'] ?? 0)
		);
	...
	/**
	 * Добавляет списочные пользовательские поля модуля в описание фильтра задач.
	 *
	 ...
	 * @return array
	 */
	private static function patchTaskUserFieldsFilterRows(array $filterRows, ?int $projectId): array
	{
		foreach (static::getTaskUserFieldEnumFilterRows($projectId) as $fieldName => $filterRow)
		{
			$filterRows[$fieldName] = $filterRow;
		}
	...
	/**
	 * Заменяет строковую фильтрацию списочных пользовательских полей на точную фильтрацию по ID значения.
	 *
	 * @param array $legacyFilter
	 * @param array $filterData
	 * @param int|null $projectId
	 *
	 * @return array
	 */
	private static function patchTaskUserFieldsProcessedFilter(
		array $legacyFilter,
		array $filterData,
		?int $projectId
	): array
	{
		foreach (static::getTaskUserFieldEnumFilterRows($projectId) as $fieldName => $filterRow)
		{
			unset($legacyFilter["%{$fieldName}"]);

			$values = array_values(array_filter(
				(array)($filterData[$fieldName] ?? []),
	...
	/**
	 * Возвращает описания списочных пользовательских полей, включенных в настройках модуля.
	 *
	 * @param int|null $projectId
	 *
	 * @return array
	 */
	private static function getTaskUserFieldEnumFilterRows(?int $projectId): array
	{
		...
		$availableFields = \Gladushenko\TaskUserFields\Task\UserFieldService::getAvailableFields();
		$filterRows = [];

		foreach (\Gladushenko\TaskUserFields\Task\UserFieldService::getDisplaySetsForProject($projectId) as $displaySet)
		{
			foreach ((array)($displaySet['fields'] ?? []) as $displayField)
			{
				...
				$items = static::getTaskUserFieldEnumItems($userField);
				...
				$filterRows[$fieldName] = [
					'id' => $fieldName,
					'name' => !empty($displayField['label'])
						? (string)$displayField['label']
						: \Gladushenko\TaskUserFields\Task\UserFieldService::getFieldSystemLabel($userField, $fieldName),
					'type' => 'list',
					'params' => [
						'multiple' => 'Y',
					],
	...
	/**
	 * Возвращает значения пользовательского списка для фильтра задач.
	 *
	 * @param array $userField
	 *
	 * @return array
	 */
	private static function getTaskUserFieldEnumItems(array $userField): array
	{
